...
if(${INDI_PUBLIC_VAR_NS}_INCLUDE_DIR)
    ...
    if(${INDI_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS MATCHES "INDI_VERSION_MAJOR[ \t]+([0-9]+)")
            set(${INDI_PUBLIC_VAR_NS}_MAJOR_VERSION "${CMAKE_MATCH_1}")
    elseif(${INDI_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS MATCHES "INDI_VERSION[^_A-Z].*\"([0-9]+)\\.([0-9]+)\\.([0-9]+)\"")
            set(${INDI_PUBLIC_VAR_NS}_MAJOR_VERSION "${CMAKE_MATCH_1}")
    else()
        message(FATAL_ERROR "failed to detect INDI version")
    endif()

    if(${INDI_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS MATCHES "INDI_VERSION_MINOR[ \t]+([0-9]+)")
            set(${INDI_PUBLIC_VAR_NS}_MINOR_VERSION "${CMAKE_MATCH_1}")
    elseif(${INDI_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS MATCHES "INDI_VERSION[^_A-Z].*\"[0-9]+\\.([0-9]+)\\.[0-9]+\"")
            set(${INDI_PUBLIC_VAR_NS}_MINOR_VERSION "${CMAKE_MATCH_1}")
    else()
        set(${INDI_PUBLIC_VAR_NS}_MINOR_VERSION "0")
    endif()
